...
from utils.storage import save_images, save_item_to_gdrive_folder
...
def get_summary_statistics(dataset):
    ...
    item_max = np.max(dataset)
    item_min = np.min(dataset)
    item_std = np.std(dataset)
    item_mean = np.mean(dataset)

    return (item_min, item_max, item_std, item_mean)

def sample_generator(sess, sample_images, input_A, input_B, x_input_A, x_input_B, dropout_rate, dropout_rate_value,
                     training_phase, data, samples_filepath, logs_path, experiment_name, e, save_to_gdrive=False,
                     phase="train", augment_data=False):

    samples = sess.run(sample_images,
                                            feed_dict={input_A: x_input_A,
                                                       input_B: x_input_B,
                                                       training_phase: False,
                                                       dropout_rate: dropout_rate_value, augment_data: False})
    sample_A_A, sample_B_B, sample_B_A, sample_A_B = samples

    sample_A_A, sample_A_B, sample_B_B, sample_B_A = data.reconstruct_original(sample_A_A), data.reconstruct_original(
        sample_A_B), data.reconstruct_original(sample_B_B), data.reconstruct_original(sample_B_A)

    input_A = data.reconstruct_original(x_input_A[0])
    input_B = data.reconstruct_original(x_input_B[0])

    filename_A = "{}/{}_face_transfer_A_{}_{}.png".format(samples_filepath, phase, experiment_name, e)
    filename_B = "{}/{}_face_transfer_B_{}_{}.png".format(samples_filepath, phase, experiment_name, e)
    #print(x_input_A.shape, sample_A_A.shape, sample_A_B.shape)
    saved_out_A = save_images([input_A, sample_A_A, sample_A_B],
                file_name=filename_A, rows=8)

    saved_out_B = save_images([input_B, sample_B_B, sample_B_A],
                file_name=filename_B, rows=8)

    if save_to_gdrive:
        save_item_to_gdrive_folder(
            file_to_save_path=filename_A,
            gdrive_folder="{}_{}".format(experiment_name, "samples_filepath"))

        save_item_to_gdrive_folder(
            file_to_save_path=filename_B,
            gdrive_folder="{}_{}".format(experiment_name, "samples_filepath"))

    return saved_out_A, saved_out_B
